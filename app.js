...
function updateWorkspaceBounds(){
 ...
 if(!shell)return;
 shell.style.marginRight=Math.max(0,(state.zoom-1)*state.canvas.w)+"px";
 shell.style.marginBottom=Math.max(0,(state.zoom-1)*state.canvas.h)+"px";
...
function setDevice(device){
 ...
 save();render();toast(d.label+" preview");
}
...
$("#newCanvas").onclick=newCanvas;$("#newLayer").onclick=addLayer;$("#gridBtn").onclick=()=>{state.grid=!state.grid;save();render()};$("#fitBtn").onclick=fitWorkspace;
...
window.addEventListener("resize",()=>{clearTimeout(window.__dfResize);window.__dfResize=setTimeout(()=>{updateWorkspaceBounds();},100)});
...
load();render();requestAnimationFrame(()=>fitWorkspace());setZoom(state.zoom);setInterval(()=>save(),5000);
})();