...
const state={
 project:"Untitled Project",theme:"dark",grid:false,zoom:1,device:"desktop",tool:"select",
...
function pointerDown(e){
 const o=page().objects.find(x=>x.id===e.currentTarget.dataset.id);if(!o)return;
 if(state.tool!=="select")return;
 e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);
 if(o.locked)return toast("Layer is locked");
 state.selected=o.id;
 const h=e.target.dataset.handle;
 if(h==="rotate"){historyPush();rotating={o,startX:e.clientX,startY:e.clientY,start:o.rotation};render();return}
 if(h){historyPush();resizing={o,h,sx:e.clientX,sy:e.clientY,x:o.x,y:o.y,w:o.w,hh:o.h};render();return}
 historyPush();drag={o,sx:e.clientX,sy:e.clientY,x:o.x,y:o.y};render();
}
window.addEventListener("pointermove",e=>{
 if(drag){const dx=(e.clientX-drag.sx)/state.zoom,dy=(e.clientY-drag.sy)/state.zoom;drag.o.x=clamp(drag.x+dx,0,state.canvas.w-drag.o.w);drag.o.y=clamp(drag.y+dy,0,state.canvas.h-drag.o.h);renderObjectLive();return}
 if(resizing){const r=resizing,dx=(e.clientX-r.sx)/state.zoom,dy=(e.clientY-r.sy)/state.zoom;let x=r.x,y=r.y,w=r.w,hh=r.hh;
  if(r.h.includes("e"))w=r.w+dx;if(r.h.includes("s"))hh=r.hh+dy;if(r.h.includes("w")){w=r.w-dx;x=r.x+dx}if(r.h.includes("n")){hh=r.hh-dy;y=r.y+dy}
  r.o.x=Math.max(0,x);r.o.y=Math.max(0,y);r.o.w=Math.max(20,w);r.o.h=Math.max(20,hh);renderObjectLive();return}
 if(rotating){const dx=e.clientX-rotating.startX,dy=e.clientY-rotating.startY;let deg=rotating.start+dx*.45-dy*.12;deg=((deg%360)+360)%360;rotating.o.rotation=Math.round(deg);renderObjectLive()}
});
window.addEventListener("pointerup",()=>{if(drag||resizing||rotating){save();drag=resizing=rotating=null;render()}});
function renderObjectLive(){const old=$("#objects").innerHTML; // Keep interactions smooth by full render; state remains authoritative.
 render();
}
function deleteSelected(){if(!selected())return;historyPush();page().objects=page().objects.filter(o=>o.id!==state.selected);state.selected=null;save();render();toast("Layer deleted")}
function duplicate(){const o=selected();if(!o)return;historyPush();const n=JSON.parse(JSON.stringify(o));n.id=uid();n.name=o.name+" Copy";n.x+=24;n.y+=24;page().objects.push(n);state.selected=n.id;save();render()}
function moveLayer(dir){const p=page(),i=p.objects.findIndex(o=>o.id===state.selected);if(i<0)return;let j=dir==="front"?p.objects.length-1:dir==="back"?0:i+(dir==="up"?1:-1);if(i===j)return;historyPush();const [o]=p.objects.splice(i,1);p.objects.splice(clamp(j,0,p.objects.length),0,o);save();render()}
function setZoom(z){state.zoom=clamp(z,.25,3);$("#zoomValue").textContent=Math.round(state.zoom*100)+"%";$("#canvasShell").style.transform=`scale(${state.zoom})`}
function setDevice(d){
 state.device=d;
 if(d!=="custom"){const x=devices[d];state.canvas.w=x.w;state.canvas.h=x.h}
 save();render();toast(d==="desktop"?"PC preview":d==="tablet"?"Tablet preview":d==="mobile"?"Phone preview":"Custom preview");
}
function updateDeviceButtons(){$$("#devicebar button[data-device]").forEach(b=>b.classList.toggle("active",b.dataset.device===state.device))}
...
function renderInspector(){
 ...
 if(!o){box.innerHTML="<div class='empty'><b>No layer selected</b>Select an object to edit its design.</div>";return}
 const r=o.responsive?.[state.device]||{};
 box.innerHTML=`
 <div class="section"><h4>Position & Size</h4>
 <div class="row"><div><label>X</label><input id="x" type="number" value="${Math.round(o.x)}"></div><div><label>Y</label><input id="y" type="number" value="${Math.round(o.y)}"></div></div>
 <div class="row"><div><label>Width</label><input id="w" type="number" value="${Math.round(o.w)}"></div><div><label>Height</label><input id="h" type="number" value="${Math.round(o.h)}"></div></div>
 <div class="row3 row"><div><label>Rotation</label><input id="rot" type="number" min="0" max="360" value="${Math.round(o.rotation)}"></div><div><label>Radius</label><input id="rad" type="number" min="0" value="${o.radius}"></div><div><label>Opacity</label><input id="op" type="number" min="0" max="1" step=".05" value="${o.opacity}"></div></div></div>
 <div class="section"><h4>Colour Customisation</h4>
 <div class="row"><div><label>Fill</label><input id="fill" type="color" value="${hex(o.fill)}"></div><div><label>Text</label><input id="textColor" type="color" value="${hex(o.textColor)}"></div></div>
 <div class="row"><div><label>Border</label><input id="stroke" type="color" value="${hex(o.stroke)}"></div><div><label>Border width</label><input id="sw" type="number" min="0" value="${o.strokeWidth}"></div></div>
 <div class="actions"><button id="gradientBtn">Gradient</button><button id="solidBtn">Solid</button></div></div>
 <div class="section"><h4>Blur Effects</h4>
 <div class="row"><div><label>Object Blur: <span id="blurVal">${o.blur}</span>px</label><input id="blur" type="range" min="0" max="40" value="${o.blur}"></div><div><label>Background Blur: <span id="backVal">${o.backdropBlur}</span>px</label><input id="backdrop" type="range" min="0" max="50" value="${o.backdropBlur}"></div></div>
 <select id="blurPreset"><option value="">Glass preset</option><option value="6">Soft Glass</option><option value="14">Medium Glass</option><option value="24">Strong Glass</option></select></div>
 ${o.type==="text"||o.type==="button"?`<div class="section"><h4>Text</h4><textarea id="txt" rows="3">${esc(o.text)}</textarea><div class="row"><div><label>Font size</label><input id="font" type="number" min="8" value="${o.fontSize}"></div><div><label>Weight</label><select id="weight">${[400,500,600,700,800].map(v=>`<option ${v==o.fontWeight?"selected":""}>${v}</option>`).join("")}</select></div></div><select id="align"><option ${o.align==="left"?"selected":""}>left</option><option ${o.align==="center"?"selected":""}>center</option><option ${o.align==="right"?"selected":""}>right</option></select></div>`:""}
 <div class="section"><h4>Responsive</h4>
 <div class="row"><div><label>Device</label><select id="respDevice"><option value="desktop">PC</option><option value="tablet">Tablet</option><option value="mobile">Phone</option></select></div><div><label>Width mode</label><select id="widthMode"><option>fixed</option><option>fluid</option><option>fill</option></select></div></div>
 <div class="row"><div><label>Device width</label><input id="rw" type="number" value="${r.w||o.w}"></div><div><label>Device height</label><input id="rh" type="number" value="${r.h||o.h}"></div></div>
 <button id="applyResponsive" class="fullbtn">Apply current size to device</button></div>
 <div class="section"><h4>Layer</h4><div class="actions"><button id="dup">Duplicate</button><button id="del">Delete</button></div><div class="actions"><button id="back">Send Back</button><button id="front">Bring Front</button></div><div class="actions"><button id="lock">${o.locked?"Unlock":"Lock"}</button><button id="hide">Hide</button></div></div>`;
 bindInspector(o);
}
function bindInspector(o){
 const bind=(id,key,type="value")=>{const e=$("#"+id);if(!e)return;e.addEventListener("change",()=>{historyPush();let v=e[type];if(["x","y","w","h","rot","rad","op","sw","font","weight"].includes(id))v=Number(v);o[key]=v;if(key==="rotation")o.rotation=((v%360)+360)%360;save();render()})};
 [["x","x"],["y","y"],["w","w"],["h","h"],["rot","rotation"],["rad","radius"],["op","opacity"],["fill","fill"],["textColor","textColor"],["stroke","stroke"],["sw","strokeWidth"],["txt","text"],["font","fontSize"],["weight","fontWeight"],["align","align"]].forEach(a=>bind(a[0],a[1]));
 $("#blur").oninput=e=>{o.blur=Number(e.target.value);$("#blurVal").textContent=o.blur;renderObjectLive()};
 $("#backdrop").oninput=e=>{o.backdropBlur=Number(e.target.value);$("#backVal").textContent=o.backdropBlur;renderObjectLive()};
 $("#blur").onchange=$("#backdrop").onchange=()=>{historyPush();save();render()};
 $("#blurPreset").onchange=e=>{if(!e.target.value)return;historyPush();o.backdropBlur=Number(e.target.value);o.blur=0;save();render()};
 $("#gradientBtn").onclick=()=>{historyPush();o.fill="linear-gradient(135deg,#3b82f6,#8b5cf6)";save();render()};
 $("#solidBtn").onclick=()=>{historyPush();o.fill="#4e8cff";save();render()};
 $("#dup").onclick=duplicate;$("#del").onclick=deleteSelected;$("#back").onclick=()=>moveLayer("back");$("#front").onclick=()=>moveLayer("front");
 $("#lock").onclick=()=>{historyPush();o.locked=!o.locked;save();render()};$("#hide").onclick=()=>{historyPush();o.hidden=true;state.selected=null;save();render()};
 const rd=$("#respDevice");rd.value=state.device;
 rd.onchange=()=>{state.device=rd.value;render()};
 $("#applyResponsive").onclick=()=>{historyPush();o.responsive=o.responsive||{desktop:{},tablet:{},mobile:{}};o.responsive[state.device]={...(o.responsive[state.device]||{}),w:Number($("#rw").value),h:Number($("#rh").value)};save();render()};
}
...
function exportModal(){
 ...
 $("#ep").onclick=()=>{const data={format:"DesignForge",version:11,...JSON.parse(snap())};download(new Blob([JSON.stringify(data,null,2)],{type:"application/json"}),safeName()+".designforge");closeModal();toast("Project exported")};
