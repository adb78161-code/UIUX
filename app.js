...
function clearSel(){selected.forEach(o=>o.classList.remove("selected"));selected.clear();primary=null; $("inspector").hidden=true}
function select(o,add=false){if(!add)clearSel();if(!o)return;selected.add(o);primary=o;o.classList.add("selected");$("inspector").hidden=false;renderInspector()}
...
function renderInspector(){
 ...
 if(!primary){$("inspector").hidden=true;return}
...
function panel(name){
 ...
 side.style.zIndex="120";sideTitle.textContent=name[0].toUpperCase()+name.slice(1);
...
function newCanvas(type){const s={website:[1440,900],landing:[1440,1024],mobile:[390,844],tablet:[768,1024],social:[1080,1080],story:[1080,1920],presentation:[1280,720],blank:[1440,900]}[type]||[1440,900];snap();canvas.innerHTML="";canvas.style.width=s[0]+"px";canvas.style.height=s[1]+"px";projectName="Untitled Project";if($("projectName")) $("projectName").textContent=projectName;closeModal();hideStart();renderInspector();toast("New canvas created")}
...
function save(){localStorage.setItem("designforge-v5-exact",JSON.stringify({name:projectName,w:canvas.offsetWidth,h:canvas.offsetHeight,html:canvas.innerHTML}));if($("saved")) $("saved").textContent="☁ Saved";toast("Project saved")}
function load(){try{const d=JSON.parse(localStorage.getItem("designforge-v5-exact")||"null");if(d){projectName=d.name||projectName;if($("projectName")) $("projectName").textContent=projectName;canvas.style.width=d.w+"px";canvas.style.height=d.h+"px";canvas.innerHTML=d.html||"";bind();hideStart()}}catch{}}

/* Navigation */
// Delegated handlers make the editor reliable on touch devices and after dynamic UI updates.
document.addEventListener("click", e=>{
  const nav=e.target.closest("[data-panel]");
  if(nav){ e.preventDefault(); e.stopPropagation(); panel(nav.dataset.panel); return; }
  const toolBtn=e.target.closest(".tool[data-tool]");
  if(toolBtn){ e.preventDefault(); const t=toolBtn.dataset.tool; tool=t; document.querySelectorAll(".tool").forEach(x=>x.classList.remove("active")); toolBtn.classList.add("active"); if(t==="text")add("text"); else if(t==="shape")add("shape"); else if(t==="frame")add("frame"); else if(t==="image")$("imageInput").click(); return; }
});
...
if($("projectBtn")) $("projectBtn").onclick=newModal;
...
$("canvasStart").addEventListener("click",()=>panel("elements"));
