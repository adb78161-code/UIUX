...
function makeObj(type,opts={}){
 const base={id:uid(),type,name:opts.name||type[0].toUpperCase()+type.slice(1),x:80,y:80,w:220,h:100,rotation:0,opacity:1,fill:"#4e8cff",stroke:"transparent",strokeWidth:0,radius:14,shadow:true,locked:false,hidden:false};
...
function render(){
 const p=currentPage();$("#pageTitle").textContent=p.name;$("#canvas").style.width=state.canvas.w+"px";$("#canvas").style.height=state.canvas.h+"px";$("#canvas").style.background=state.canvas.bg;$("#canvas").classList.toggle("grid",state.grid);
...
function renderObj(o){
 ...
 Object.assign(el.style,{left:o.x+"px",top:o.y+"px",width:o.w+"px",height:o.h+"px",transform:`rotate(${o.rotation||0}deg)`,opacity:o.opacity,background:o.fill==="transparent"?"transparent":o.fill,border:`${o.strokeWidth||0}px solid ${o.stroke||"transparent"}`,borderRadius:(o.radius||0)+"px",boxShadow:o.shadow?"0 10px 28px #18324a22":"none"});
...
function bindInspector(o){
 const map={iName:"name",iX:"x",iY:"y",iR:"rotation",iW:"w",iH:"h",iFill:"fill",iOpacity:"opacity",iStroke:"stroke",iRadius:"radius",iSW:"strokeWidth",iShadow:"shadow",iText:"text",iFont:"fontSize",iWeight:"fontWeight",iTextColor:"textColor",iAlign:"align"};
...
$("#exportBtn").onclick=exportProject;$("#projectNameBtn").onclick=()=>{const n=prompt("Project name",state.project);if(n){saveHistory();state.project=n;save();toast("Project renamed")}};
